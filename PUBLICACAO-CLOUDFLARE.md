...
5. **Chave de criptografia** (protege os tokens do Mercado Livre no banco): gerar no seu terminal
   ...
   e guardar só no GitHub (passo 7). Se for perdida, será preciso reconectar a conta do Mercado Livre.
6. **Login (Cloudflare Access)** — antes da primeira publicação, porque o gestor não aceita outro login: *Zero Trust* (no menu da conta; na primeira vez, escolha um nome de equipe e o plano Free) → *Access → Applications → Add an application → Self-hosted*. Domínio: `gestor-promocoes.<subdomínio>.workers.dev`. Política *Allow* com *Include → Emails* = o seu e-mail. Método de login: *One-time PIN*. Anotar o **Application Audience (AUD) Tag** (na aplicação) e o **team domain** (`<equipe>.cloudflareaccess.com`, em *Zero Trust → Settings*). Os nomes no painel podem variar um pouco.
7. **GitHub → Settings → Secrets and variables → Actions**:
   - *Secrets*: `CLOUDFLARE_API_TOKEN` (passo 4), `ML_ENCRYPTION_KEY` (passo 5).
   - *Variables*: `CLOUDFLARE_ACCOUNT_ID`, `CF_WORKER_NAME=gestor-promocoes`, `CF_D1_DATABASE_NAME=gestor-promocoes`, `CF_D1_DATABASE_ID`, `PUBLIC_ORIGIN=https://gestor-promocoes.<subdomínio>.workers.dev`, `ML_CLIENT_ID=5716651302555823`, `CF_ACCESS_TEAM_DOMAIN` e `CF_ACCESS_AUD` (passo 6). Faltando qualquer variável, o build falha de propósito e nada é publicado.
8. **Primeira publicação**: *Actions → Publicar na Cloudflare → Run workflow*. Cria as tabelas no D1 e o Worker. Ao abrir o endereço, o Access pede o seu e-mail e envia um código.
