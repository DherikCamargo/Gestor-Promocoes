...
// Revalida tudo no servidor (lib/participate.ts). A tela só informa o preço escolhido em DEAL e
// PRICE_DISCOUNT; limites, margem e preço aplicado são conferidos aqui.
...
export async function POST(request:Request){
 ...
 let body:{itemId?:unknown;refId?:unknown;price?:unknown;days?:unknown};try{body=await request.json()}catch{return json({error:'Pedido inválido.'},400)}
 ...
 // Preço escolhido (DEAL/PRICE_DISCOUNT) e duração do desconto individual; limites conferidos no servidor.
 const price=body.price,days=body.days;
 if(price!==undefined&&(typeof price!=='number'||!Number.isFinite(price)||price<=0||price>1000000))return json({error:'Preço inválido.'},400);
 if(days!==undefined&&(typeof days!=='number'||!Number.isInteger(days)||days<1||days>14))return json({error:'Duração inválida (1 a 14 dias).'},400);
 const choice=typeof price==='number'?{price,days:typeof days==='number'?days:undefined}:undefined;
 const db=runtime().DB;
 ...
 try{
  ...
  const result=await participateOffer({api,itemId,refId,overrides,rules:settings.rules,log,choice});
