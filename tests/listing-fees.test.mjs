...
test('desconto na tarifa lido de discount_meli_boost_amount (exemplo da documentação)',()=>{
 assert.deepEqual(activeOffer([smart],smart.ref_id,3245),{status:'identified',name:'Desconto no Pix',type:'SMART',feeDiscount:555,meliPercentage:8,mlSubsidy:400});
});
...
test('oferta sem boosted_offer e sem co-participação não tem desconto na tarifa',()=>{
 ...
 assert.deepEqual(activeOffer([deal],'OFFER-1',148.35),{status:'identified',name:'Top sellers',type:'DEAL',feeDiscount:0,meliPercentage:null,mlSubsidy:0});
});

// Caso real (conferência de 24/09/2026): a Central mostra "Reduzimos R$ 6,91 das suas tarifas",
// a API envia só meli_percentage 3,5 (arredondado). Pedido: exibir o subsídio calculado sobre o preço bruto.
test('co-participação real MLB3575190873: subsídio = 3,5% × R$ 199,90 = 6,9965 → R$ 6,99 (para baixo)',()=>{
 ...
 assert.deepEqual(activeOffer([candidata,queima],queima.ref_id,109),{status:'identified',name:'Queima de inverno Full',type:'SMART',feeDiscount:0,meliPercentage:3.5,mlSubsidy:6.99});
});
...
test('subsídio sem preço original fica não informado, nunca zero',()=>{
 const semOriginal={type:'SMART',ref_id:'OFFER-2',status:'started',price:109,meli_percentage:3.5};
 assert.equal(activeOffer([semOriginal],'OFFER-2',109).mlSubsidy,null);
});

test('subsídio sempre arredonda para baixo no centavo',()=>{
 const o={type:'SMART',ref_id:'OFFER-3',status:'started',price:112.43,original_price:199.9,meli_percentage:3.96};
 assert.equal(activeOffer([o],'OFFER-3',112.43).mlSubsidy,7.91);
 assert.equal(activeOffer([{...o,meli_percentage:0.57,original_price:100}],'OFFER-3',112.43).mlSubsidy,0.57);
});
